      ...
      Releases.all().then(function(cdata) {
        ...
        if (cdata.length > 0) {
          ...
          angular.forEach($scope.magazines, function(value, key) {
            ...
            $scope.totalrows.push({
              "image": $scope.magazines[key].image,
              "id": $scope.magazines[key].id,
              "introduction": $scope.magazines[key].introduction,
              "article": $scope.magazines[key].extracted_file.article,
              "pages": $pagearray,
              'extracted_file': $scope.magazines[key].extracted_file
            });
          });
          $ionicLoading.hide();
        } else {
